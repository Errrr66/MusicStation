...
const pieFrame = computed(() => {
  ...
  const pulse = Math.sin(pulsePhase) * 0.03 + 1;
  ...
  for (let i = 0; i < animatedValues.value.length; i++) {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        const adjustedInnerRadius = innerRadius * pulse;

        if (dist <= adjustedRadius && dist >= adjustedInnerRadius) {
...
<template>
  <div class="matrix-pie-container">
    ...
    >
      ...
      >
        <defs>
          <filter id="pie-pixel-glow" x="-50%" y="-50%" width="200%" height="200%">
