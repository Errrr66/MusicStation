...
function buildCells() {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const edgeRatio = (dist - innerRadius) / (radius - innerRadius);
      const color = palette[segmentIndex % palette.length];
      const brightness = 0.5 + edgeRatio * 0.5;
...
function drawCells(revealProgress: number, _pulse: number, elapsed = 0) {
  ...
  for (const cell of cells) {
    ...
    drawMatrixCell(ctx, cell.x, cell.y, cellRadius, cell.pattern as any, cell.color);
  }
...
function drawActiveGlow() {
  ...
  for (const cell of cells) {
    ...
    ctx.beginPath();
    ctx.arc(cell.x, cell.y, cellRadius * 2.2, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
}
...
function resolveDataIndex(event: MouseEvent): number {
  ...
  const dy = y - centerY;
  const dist = Math.sqrt(dx * dx + dy * dy);
  const radius = (Math.min(cols, rows) * (size + gap)) / 2 - (size + gap) * 1.5;
  const innerRadius = radius * 0.42;
